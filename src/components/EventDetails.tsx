...
const EventDetails = ( { event } : EventDetailsProps): ReactElement => {
  ...
  return (
    <div
      className="flex flex-1 flex-col p-10 gap-8"
    >
      ...
      <div
        className="flex flex-col w-full md:flex-row gap-12"
      >
        <div className="flex flex-row h-60 md:w-6/12 md:h-96">
          <div
            className="flex-1 bg-no-repeat bg-isec bg-cover rounded-l-2xl object-cover"
          />
          <div className="flex-1 bg-sga-red rounded-r-2xl text-white text-right p-4">
            <div className="flex flex-col justify-around md:justify-between h-full">
              <div>
                <div className="font-sans font-bold text-5xl">{event.startTime.toLocaleDateString('en-US', {day: 'numeric'})}</div>
                <div className="text-l">
                  <span className="font-sans font-bold">{event.startTime.toLocaleDateString('en-US', {month: 'long'})}</span>
                  <span className="font-montserrat pl-1">'{event.startTime.toLocaleDateString('en-US', {year: 'numeric'})}</span>
                  <br />
                  <span className="font-montserrat">{event.startTime.toLocaleDateString('en-US', {weekday: 'long'})}</span>
                </div>
              </div>
